		...
		{#each Object.entries($favorites) as [url, title]}
			<li class="mt-4">
				...
				<div class="flex flex-row mt-1">
					<a
						href={url}
						class="flex cursor-pointer items-center text-sm font-bold leading-6 text-pink-500 hover:text-pink-700 active:text-pink-900"
					>
						Full Article
					</a>
					<!-- svelte-ignore a11y-click-events-have-key-events -->
					<div
						on:click={removeFav(url)}
						class="ml-3 flex flex-row cursor-pointer items-center text-sm font-bold leading-6 text-pink-500 hover:text-pink-700 active:text-pink-900"
					>
